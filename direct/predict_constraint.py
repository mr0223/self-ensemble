...
def make_one_constraint_withinput(
    ...
    line_ids = bert_tokenizer(line, return_tensors="pt").input_ids.to(device)
...
def make_one_constraint_withinput_multi(
    ...
    one_constraint_raws = []
    for one_line in one_line_summarized:
...
def make_whole_constraint_withinput_multi(
    whole_line, model, return_num, bert_tokenizer, device
    ):
...
def main():
    ...
    parser.add_argument('--output_ext', type=str, help="indirect or direct or w.indirect or w.direct")
    ...
    predict_input_path = f"predict_input/baseline/b{return_num}.{data_type}.{output_ext}.csv"
    constraint_path_majority = f"constraint/both/b{return_num}.majority.{data_type}.{output_ext}.pkl"
    constraint_path_majority_p = f"constraint/positive/b{return_num}.majority.{data_type}.{output_ext}.pkl"
    constraint_path_majority_n = f"constraint/negative/b{return_num}.majority.{data_type}.{output_ext}.pkl"
    # constraint_path_union = f"constraint/both/b{return_num}.union.{data_type}.{output_ext}.pkl"
    ...
    constraint_raws_majority, constraint_nums_majority, \
        constraint_raws_union, constraint_nums_union \
            = make_whole_constraint_withinput_multi(model_input_lines, model, return_num, bert_tokenizer, device)
