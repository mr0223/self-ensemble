...
def _generate_beam_search(
    ...
    while cur_len < max_length:
        ...
        model_inputs["encoder_outputs"] = (encoder_outputs[0],) + past[0][1:]

        model_inputs.pop("past_key_values")
        model_inputs["past_key_values"] = past[1]

        outputs = self(**model_inputs)  # (batch_size * num_beams, cur_len, vocab_size)
        ## changed ##
        if len(outputs)==5:
            shaped_outputs_101 = Tensor([[False for _ in range(len(outputs[2][0]))] for _ in range(len(outputs[2]))]).to(outputs[2].device)
            shaped_outputs_1 = ((outputs[2], shaped_outputs_101), outputs[1])
            shaped_outputs = (outputs[0], shaped_outputs_1, outputs[2])
        elif len(outputs)==4:
            shaped_outputs_101 = Tensor([[False for _ in range(len(outputs[2][0]))] for _ in range(len(outputs[2]))]).to(outputs[2].device)
            shaped_outputs_1 = ((outputs[2], shaped_outputs_101), outputs[1])
            shaped_outputs = (outputs[0], shaped_outputs_1, outputs[2], outputs[3])
        else:
            print("output size is not 4,5")
            shaped_outputs = outputs
        ...
        if do_sample:
            ...
            pick_scores, pick_tokens, constraints, num_mets = topk_huggingface(timestep=cur_len,
                                                                               ...
                                                                               temp_past=past,
                                                                               encoder_outputs=encoder_outputs,    ## changed ##
        ...
        if past is not None:
            ## changed ##
            past0_cache = past[0]
            past = self._reorder_cache(past[1], beam_idx)
            past = (past0_cache, past)
